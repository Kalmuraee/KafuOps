...
| Call an LLM with a sanitized context bundle | 🟡 | `src/llm/orchestrator.ts` supports two providers: **OpenAI** (`OPENAI_API_KEY`) and **Anthropic Claude** (`ANTHROPIC_API_KEY`). Choose with `llm.provider: openai \| anthropic` in `.kafuops.yml`. `llm.trigger_mode: 'off'` is a hard dry-run gate. Without a working key the orchestrator falls back to deterministic offline heuristics. Every call audited to `.kafuops/audit/model-calls/`. |
| Generate a small patch and regression test | 🟡 | Patch generation runs through whichever provider is selected. The deterministic dry-run intentionally returns an empty diff so audit logs never contain a fake fix. End-to-end fix quality on a real broken app is **not yet validated by tests in this repo**. |
